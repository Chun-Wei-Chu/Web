(function() {
  ...
  $(function() {
    var arc, color, cr, cs, donut, h, hash, pb, pl, pr, pt, r, w, dataFrom, _ref, barText, Circle_width;
    Circle_width = __width__;
    dataFrom = __dataFrom__;
    barText = __barText__;
    _ref = [35, 20, 20, 20], pt = _ref[0], pl = _ref[1], pb = _ref[2], pr = _ref[3];
    w = (Circle_width - (pl + pr)) / 2;
    h = w;
    r = (w - (pt * 2)) / 2;
    cs = 0.65;
    cr = r * cs;
    ...
    return d3.json(dataFrom, function(json) {
      ...
      vis.append('svg:text').text(barText).attr('transform', "translate(" + (w / 2) + "," + (h + 30) + ")").attr('text-anchor', 'middle');
